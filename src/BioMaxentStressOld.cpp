...
namespace NetworKit {
    ...
    BioMaxentStress::BioMaxentStress(
                    ...
                    ) : q(q), alpha(alpha), alphaReduction(.3), finalAlpha(.001), convThreshold(0.001*0.001), coordinatesProvided(false), probabilityProvided(true), fastComputation(false), /*maxSolvesPerAlpha(solvesPerAlpha),*/ /*probability(weights),*/ dim(3), hasRun(false), loggingFrequency(loggingFrequency)
    {
        G = Graph(numNodes, true, false);
        ...
        {
            G.addEdge(edges[i].first, edges[i].second, distances[i]);
        }
        ...
        BioMaxentStress::probability = sortedWeights;

        //vertexCoordinates = initialCoordinates;
        vertexCoordinates = std::vector<Point<double>>(G.numberOfNodes(), Point<double>(dim));
...
void BioMaxentStress::run(uint64_t maxSolvesPerAlpha) {
    ...
    std::cout << "alphared "<< alphaReduction << std::endl;
    std::cout << "finalpha "<< finalAlpha << std::endl;
    std::cout << "thresh "<< convThreshold << std::endl;
    std::cout << "coordprov " << coordinatesProvided << std::endl;
    std::cout << "probprov " << probabilityProvided << std::endl;
    std::cout << "maxSolv " << maxSolvesPerAlpha << std::endl;
...
#pragma omp parallel for
	...
	while (!converged) { // Run until converged (usually when currentAlpha == finalAlpha)
		...
		for (count numSolves = 0; numSolves < maxSolvesPerAlpha; ++numSolves) { // solve up to maxSolvesPerAlpha linear systems
            ...
            INFO("Computing coordinate laplacian");
			t.start();
...
#pragma omp parallel for
            ...
            ////////////////////////////////////////////
            //
            bool foundANaN = false;
            for(uint64_t d=0; d<dim; ++d)
            {
                for(uint64_t i=0; i<G.numberOfNodes(); ++i)
                {
                    if(std::isnan(newCoordinates[d][i]) || std::isinf(newCoordinates[d][i]))
                    {
                        foundANaN = true;
                        //std::cout << oldCoordinates[d][i] << " " << newCoordinates[d][i] << std::endl;
                    }
                }
            }
            if(foundANaN)
            {
                std::cout << "found a NaN in the new coordinates" << std::endl;
